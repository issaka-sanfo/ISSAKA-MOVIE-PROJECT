...
export const SeenMovie = (props: ISeenMovieProps) => {
  ...
  const { seenMovieList, match, loading, totalItems } = props;
  return (
    <div>
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  Movie <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  Movie User <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {seenMovieList.map((seenMovie, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{seenMovie.movieId ? <Link to={`movie/${seenMovie.movieId}`}>{seenMovie.movieId}</Link> : ''}</td>
                  <td>{seenMovie.movieUserId ? <Link to={`movie-user/${seenMovie.movieUserId}`}>{seenMovie.movieUserId}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${seenMovie.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${seenMovie.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
      ...
      <div className={seenMovieList && seenMovieList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ seenMovie }: IRootState) => ({
  ...
  totalItems: seenMovie.totalItems
});
